        ...
        implements PlanOptimizer
{
            ...
            extends SimplePlanRewriter<Void>
    {
        ...
        public PlanNode visitPatternRecognition(PatternRecognitionNode node, RewriteContext<Void> rewriteContext)
        {
            ...
            /*
             * Bucketized path:
             * build a conservative bucket sketch and rewrite the input by a filter over candidate windows.
             * If we can build a valid bucketized rewrite, we prefer it here.
             */
            if (patternWindow != null) {
                BucketizedPrefilterBuilder bucketizedBuilder =
                        new BucketizedPrefilterBuilder(new BucketizedPrefilterConfig(patternWindow.windowSize()));

                BucketizedPrefilterBuilder.BucketizedPrefilterPlanSketch sketch =
                        bucketizedBuilder.buildSketch(
                                t,
                                firstSubsequenceLabel,
                                lastSubsequenceLabel,
                                subsequence,
                                independent,
                                dependent,
                                Optional.of(patternWindow));

                bucketizedBuilder.printSketch(sketch);

                PlanNode bucketizedRewrite = buildBucketizedRewrite(node, inputRelation, t, sketch, context);
                if (bucketizedRewrite != null) {
                    return bucketizedRewrite;
                }
            }
    ...
    private static PlanNode buildBucketizedRewrite(
            PatternRecognitionNode node,
            PlanNode inputRelation,
            Symbol orderKey,
            BucketizedPrefilterBuilder.BucketizedPrefilterPlanSketch sketch,
            Context context)
    {
        if (orderKey == null || sketch == null || sketch.candidateWindows().isEmpty()) {
            return null;
        }

        Expression filterPredicate = buildBucketizedFilterPredicate(orderKey, sketch);
        if (filterPredicate == null) {
            return null;
        }

        FilterNode filteredInput = new FilterNode(
                context.idAllocator().getNextId(),
                inputRelation,
                filterPredicate);

        return replaceChildren(node, List.of(filteredInput));
    }

    private static Expression buildBucketizedFilterPredicate(
            Symbol orderKey,
            BucketizedPrefilterBuilder.BucketizedPrefilterPlanSketch sketch)
    {
        List<Expression> windowPredicates = new ArrayList<>();

        for (BucketizedPrefilterBuilder.BucketWindow window : sketch.candidateWindows()) {
            long startInclusive = sketch.layout().bucketStart(window.startBucketId());
            long endExclusive = sketch.layout().bucketEnd(window.endBucketId());

            Expression lowerBound = new Comparison(
                    Comparison.Operator.GREATER_THAN_OR_EQUAL,
                    orderKey.toSymbolReference(),
                    new Constant(orderKey.type(), startInclusive));

            Expression upperBound = new Comparison(
                    Comparison.Operator.LESS_THAN,
                    orderKey.toSymbolReference(),
                    new Constant(orderKey.type(), endExclusive));

            windowPredicates.add(new Logical(
                    Logical.Operator.AND,
                    List.of(lowerBound, upperBound)));
        }

        if (windowPredicates.isEmpty()) {
            return null;
        }

        if (windowPredicates.size() == 1) {
            return windowPredicates.get(0);
        }

        return new Logical(Logical.Operator.OR, windowPredicates);
    }
